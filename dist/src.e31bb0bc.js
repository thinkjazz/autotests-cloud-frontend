...
function connect() {
  var socket = new _sockjs.default("http://localhost:8080/gs-guide-websocket"); // add exception
...
function showGreeting(message) {
  document.querySelector("#greetings").append("<tr><td>" + message + "</td></tr>");
}
...
var initForm = function initForm() {
  ...
  function submitForm(event) {
    ...
    if (!!values.steps && values.title) {
      ...
      console.log(values);
      var a = stompClient.send("/1app/hello", {}, JSON.stringify(values));
      console.log(a); // const response = sendData(url, JSON.stringify(values));

      headerTextarea.innerHTML = "In progress..."; // add exception

      codeBlock.classList.remove("hidden");
      mainForm.classList.add("hidden");
      iframeBlock.classList.remove("hidden"); // response.then((resp) => {
      ...
      iframeBlock.innerHTML = "<iframe id=\"telegram-post-autotests_cloud-17\" class=\"telegram-iframe w-full h-full h-80\"\n          src=\"https://t.me/autotests_cloud_chat/596?embed=1&dark=1\" frameborder=\"0\" scrolling=\"yes\"></iframe>"; // src="https://t.me/autotests_cloud/${resp}?embed=1&dark=1" frameborder="0" scrolling="yes"></iframe>`;
